...
def main():
  ...
  try:
    ...
    # itopk_size accepts either a scalar or a list — when it's a list,
    # the example sweeps each value and prints throughput + recall per row.
    itopk_size_raw = config.get('itopk_size', 32)
    itopk_sizes = ([int(v) for v in itopk_size_raw]
                   if isinstance(itopk_size_raw, (list, tuple))
                   else [int(itopk_size_raw)])
  ...
  # Generate ground truth once (re-used for every itopk_size below)
  ...
  for itopk in itopk_sizes:
    ...
    for _ in range(warmup_runs):
      _, _ = vf.search(queries_np_gpu, query_labels_np_gpu, itopk)

    # Timed runs
    start_time = time.perf_counter()
    for _ in range(num_runs):
      neighbors, distances = vf.search(queries_np_gpu, query_labels_np_gpu, itopk)
