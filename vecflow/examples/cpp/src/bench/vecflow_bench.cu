...
void create_bitmap_filter_fast(raft::resources const& handle,
                             ...
                             const raft::device_vector_view<const uint32_t, int64_t>& query_labels_view, // Use view
                             int64_t n_queries,
                             int64_t n_database,
                             const raft::device_matrix_view<uint32_t, int64_t>& bitmap) { // Use view
...
double cagra_search_with_post_processing(shared_resources::configured_raft_resources& dev_resources,
  ...
  printf("=== Performing CAGRA Search (itopk=%d) with Post-Processing ===\n", itopk_size);
  cagra::search_params search_params;
  search_params.itopk_size = itopk_size;
...
int main(int argc, char** argv) {
	...
	try {
		...
		ivf_cagra_index_fname = config["ivf_cagra_index_fname"];
		ivf_bfs_index_fname = config["ivf_bfs_index_fname"];
  ...
  vecflow::index<float> idx;
	auto stream = raft::resource::get_cuda_stream(idx.res);
	auto d_data = raft::make_device_matrix<float, int64_t>(idx.res, N, dim);
	raft::copy(d_data.data_handle(), h_data.data(), N * dim, stream);

	auto d_queries = raft::make_device_matrix<float, int64_t>(idx.res, Nq, dim);
	...
	auto d_query_labels_main = raft::make_device_vector<uint32_t, int64_t>(idx.res, Nq);
	raft::copy(d_query_labels_main.data_handle(), h_query_labels.data(), Nq, stream);

  auto gt_neighbors = raft::make_device_matrix<uint32_t, int64_t>(idx.res, Nq, topk);
	...
	if (std::find(algorithms_to_run.begin(), algorithms_to_run.end(), "vecflow") != algorithms_to_run.end()) {
		...
		auto vecflow_neighbors = raft::make_device_matrix<uint32_t, int64_t>(idx.res, Nq, topk);
		auto vecflow_distances = raft::make_device_matrix<float, int64_t>(idx.res, Nq, topk);
		...
		for (int current_itopk : itopk_sizes) {
			...
			double recall = compute_recall(idx.res, vecflow_neighbors.view(), gt_neighbors.view());
  ...
  cagra::index<float, uint32_t> cagra_index(idx.res);
	if (std::find(algorithms_to_run.begin(), algorithms_to_run.end(), "cagra_post_processing") != algorithms_to_run.end()) {
		...
		build_cagra_index(idx.res, cagra_index, raft::make_const_mdspan(d_data.view()),
						          full_cagra_index_fname, graph_degree);

		printf("\n=== CAGRA Search with Post-Processing Benchmarking ===\n");
		auto filtered_neighbors_pp = raft::make_device_matrix<uint32_t, int64_t>(idx.res, Nq, topk);
		for (int current_itopk : itopk_sizes) {
			printf("-- Running CAGRA Search + PP (itopk=%d) --\n", current_itopk);
			double qps = cagra_search_with_post_processing(idx.res, cagra_index,
										raft::make_const_mdspan(d_queries.view()),
			...
			double recall = compute_recall(idx.res, filtered_neighbors_pp.view(), gt_neighbors.view());
			printf("  - QPS: %.2f, Recall@%d: %.4f\n", qps, topk, recall);
    ...
    auto filtered_neighbors_inline = raft::make_device_matrix<uint32_t, int64_t>(idx.res, Nq, topk);
    for (int current_itopk : itopk_sizes) {
      printf("-- Running CAGRA Search + Inline Filter (itopk=%d) --\n", current_itopk);
      double qps = cagra_search_inline_filtering(idx.res, cagra_index,
                            raft::make_const_mdspan(d_queries.view()),
                            d_query_labels_main.view(), label_data_vecs,
                            current_itopk, topk, num_runs, warmup_runs,
                            filtered_neighbors_inline.view());
      double recall = compute_recall(idx.res, filtered_neighbors_inline.view(), gt_neighbors.view());
      printf("  - QPS: %.2f, Recall@%d: %.4f\n", qps, topk, recall);
