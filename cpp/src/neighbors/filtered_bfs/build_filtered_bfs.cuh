...
void build_filtered_bfs_core(raft::resources const& res,
  ...
  *idx = cuvs::neighbors::ivf_flat::index<data_t, idx_t>(
    res, cuvs::distance::DistanceType(metric), n_lists, false, true, dim);

  cuvs::neighbors::ivf_flat::detail::fill_variable_size_index(res,
                                                              idx,
                                                              dataset,
                                                              index_map,
                                                              label_size,
...
void build_filtered_bfs_impl(
  ...
  raft::device_vector_view<uint32_t, int64_t> label_offset)
{
  detail::build_filtered_bfs_core(res, idx, dataset, index_map, label_size, label_offset);
}
}  // namespace cuvs::neighbors