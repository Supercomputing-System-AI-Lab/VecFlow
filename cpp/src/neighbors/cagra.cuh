...
/** Typed helper for VecFlow's filtered_search path. */
template <typename T, typename IdxT, typename CagraSampleFilterT>
void filtered_search_with_filtering(
  raft::resources const& res,
  const search_params& params,
  const index<T, IdxT>& idx,
  raft::device_matrix_view<const T, int64_t, raft::row_major> queries,
...
/** VecFlow filtered_search dispatcher: routes by filter dynamic type. */
template <typename T, typename IdxT>
                     ...
                     const uint32_t* query_labels_second_ptr   = nullptr)
{
  ...
  try {
    using bitset_filter_type = cuvs::neighbors::filtering::bitset_filter<uint32_t, int64_t>;
    auto& sample_filter      = dynamic_cast<const bitset_filter_type&>(sample_filter_ref);
    auto sample_filter_copy  = sample_filter;
  ...
  RAFT_FAIL("Unsupported sample filter type for filtered_search");
}
