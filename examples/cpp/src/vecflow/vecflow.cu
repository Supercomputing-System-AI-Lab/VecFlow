...
struct CombinedIndices {
  int cagra_labels;
  int64_t cagra_total_rows;
  int bfs_labels;
  int64_t bfs_total_rows;
...
template<typename T>
auto build_graphs(shared_resources::configured_raft_resources& res,
  ...
  if (bfs_labels > 0) {
    ...
    ivf_flat::index<T, int64_t> bfs_index(res,
...
template<typename T>
void load_index(shared_resources::configured_raft_resources& res,
                raft::device_matrix_view<const T, int64_t> dataset,
                cagra::index<T, uint32_t>& cagra_index,
                ivf_flat::index<T, int64_t>& bfs_index,
                const CombinedIndices& metadata,
                int graph_degree,
                std::string graph_fname,
                std::string bfs_fname) {
  // Build index
  int64_t total_cagra_rows = metadata.cagra_index_map.extent(0);
  auto graph = raft::make_device_matrix<uint32_t, int64_t>(res, total_cagra_rows, graph_degree);
  raft::resource::sync_stream(res);
  ...
  if (graph_file.good()) {  
    ...
    load_matrix_from_ibin(res, graph_fname, graph.view());
    cagra_index.update_graph(res, raft::make_const_mdspan(graph.view()));
    if (bfs_file.good()) {
  ...
  std::cout << "  Number of labels: " << metadata.cagra_labels << std::endl;
  ...
  std::cout << "  Number of labels: " << metadata.bfs_labels << std::endl;
  std::cout << "  Number of rows:  " << metadata.bfs_total_rows << std::endl;
}
...
void search_main(shared_resources::configured_raft_resources& res,
  ...
  convert_neighbors_to_uint32(res,
                              ...
                              n_bfs_queries,
                              topk);
  auto final_neighbors = raft::make_device_matrix<uint32_t, int64_t>(res, n_queries, topk);
  raft::resource::sync_stream(res);
  ...
  raft::copy(final_neighbors.data_handle(),
            ...
            topk * n_cagra_queries,
            raft::resource::get_cuda_stream(res));
  raft::resource::sync_stream(res);
  raft::copy(final_neighbors.data_handle() + (n_cagra_queries) * topk,
            temp_bfs_neighbors.data_handle(),
            topk * n_bfs_queries,
            raft::resource::get_cuda_stream(res));
  raft::resource::sync_stream(res);
  ...
  std::copy(cagra_indices.begin(), cagra_indices.end(), std::back_inserter(valid_query_indices));
  std::copy(bfs_indices.begin(), bfs_indices.end(), std::back_inserter(valid_query_indices));

  auto h_neighbors = raft::make_host_matrix<uint32_t, int64_t>(n_queries, topk);
  raft::copy(h_neighbors.data_handle(), final_neighbors.data_handle(), final_neighbors.size(), raft::resource::get_cuda_stream(res));
  raft::resource::sync_stream(res);
  ...
  compute_recall(h_neighbors.view(), gt_indices, valid_query_indices);
}
...
int main(int argc, char** argv) {
  ...
  auto metadata = build_graphs<float>(res,
  ...
  auto bfs_index = ivf_flat::index<float, int64_t>(res,
                                              cuvs::distance::DistanceType::L2Unexpanded,
                                              metadata.cat_freq.size(),
                                              false,
                                              true,
                                              dataset.extent(1));
  load_index<float>(res,
                    raft::make_const_mdspan(dataset.view()),
                    cagra_index,
                    bfs_index,
                    metadata,
