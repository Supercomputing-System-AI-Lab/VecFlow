...
void compute_recall(const raft::host_matrix_view<uint32_t, int64_t>& neighbors,
  ...
  int n_ivf_queries = n_queries - n_single_queries - n_double_queries;
  ...
  std::ofstream outfile("/u/cmo1/Filtered/cuvs/examples/cpp/src/result/yfcc_recall_results.csv");
  if (!outfile.is_open()) {
  ...
  for (int i = 0; i < neighbors.extent(0); i++) {
    ...
    } else if (i < n_single_queries + n_double_queries) {
  ...
  std::cout << "Results have been written to yfcc_recall_results.csv" << std::endl;
}
...
void cagra_ivf_build_search_variants(shared_resources::configured_raft_resources& dev_resources,
                                    ...
                                    const std::vector<int>& cat_freq,
                                    const std::vector<int>& query_freq,
                                    int single_query_itopk_size = 64,
                                    int double_query_itopk_size = 64,
  ...
  for (int64_t i = 0; i < query_label_vecs.size(); i++) {
    if (query_label_vecs[i].size() == 1) {
      ...
      // if (min(cat_freq[label1], cat_freq[label2]) <= specificity_threshold) {
      //   int label = (cat_freq[label1] <= cat_freq[label2]) ? label1 : label2;
      //   h_ivf_filtered_labels[current_ivf] = label;
      //   ivf_query_indices[current_ivf] = i;
      //   current_ivf++;
      // } else {
      //   int label = (cat_freq[label1] <= cat_freq[label2]) ? label1 : label2;
  ...
  int n_queries = n_single_queries + n_double_queries + n_ivf_queries;
  ...
  std::cout << "\n=== CAGRA Single Label Search Performance ===" << std::endl;
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    cagra::filtered_search(dev_resources, 
                          ...
                          combined_indices.label_size.view(),
                          combined_indices.label_offset.view());
    auto double_dev_resources = dev_resources;
    cagra::filtered_search(double_dev_resources, 
                          filtered_double_search_params,
                          cagra_index,
                          double_queries.view(),
                          double_query_neighbors.view(),
                          ...
                          combined_indices.label_offset.view(),
                          filter);
    auto ivf_dev_resources = dev_resources;
    search_filtered_ivf(ivf_dev_resources,
                        filtered_ivf_index,
                        ivf_queries.view(),
                        ivf_query_labels.view(),
                        combined_indices.label_size.view(),
                        ...
                        cuvs::distance::DistanceType::L2Unexpanded);
    raft::resource::sync_stream(dev_resources);
    raft::resource::sync_stream(double_dev_resources);
    raft::resource::sync_stream(ivf_dev_resources);
    auto end = std::chrono::system_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
  ...
  double total_queries = n_single_queries + n_double_queries + n_ivf_queries;
  double qps = (total_queries * 1000.0) / avg_time_ms;
  std::cout << "\n=== Combined Search Performance ===" << std::endl;
  std::cout << "Configuration:" << std::endl;
  std::cout << "  Specificity threshold: " << specificity_threshold << std::endl;
  std::cout << "  CAGRA iTopK (single):  " << single_query_itopk_size << std::endl;
  std::cout << "  CAGRA iTopK (double):  " << double_query_itopk_size << std::endl;
  std::cout << "\nQuery Statistics:" << std::endl;
  std::cout << "  CAGRA Single Label:  " << std::setw(8) << n_single_queries << " queries" << std::endl;
  ...
  raft::copy(final_neighbors.data_handle() + n_single_queries * topk,
            ...
            raft::resource::get_cuda_stream(dev_resources));
  raft::resource::sync_stream(dev_resources);
  raft::copy(final_neighbors.data_handle() + (n_single_queries + n_double_queries) * topk,
            temp_ivf_neighbors.data_handle(),
            topk * n_ivf_queries,
            raft::resource::get_cuda_stream(dev_resources));
  raft::resource::sync_stream(dev_resources);
  ...
  compute_recall(h_neighbors.view(), gt_indices, valid_query_indices, n_single_queries, n_double_queries, query_label_vecs);
}
