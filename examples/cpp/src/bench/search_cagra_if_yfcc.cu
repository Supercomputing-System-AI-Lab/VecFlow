...
struct CombinedIndices {
  ...
  raft::device_matrix<int64_t, int64_t> query_labels;
};
...
struct cagra_filter {
  ...
  raft::device_matrix_view<int64_t, int64_t> query_labels_;
  ...
  cagra_filter(raft::device_vector_view<uint32_t, int64_t> data_labels,
              ...
              raft::device_matrix_view<int64_t, int64_t> query_labels,
    ...
    const uint32_t sample_ix) const 
  {
    ...
    int query_label = query_labels_(global_query_ix, 0);
    if (!binary_search(query_label, left, right)) {
      return false;
    }
    
    // Search for second category if needed
    query_label = query_labels_(global_query_ix, 1);
    if (query_label != -1) {
      if (!binary_search(query_label, left, right)) {
        return false;
      }
...
auto load_and_combine_indices(raft::resources const& res, 
  ...
  std::vector<int64_t> h_query_labels(n_queries*2);
  for (size_t i = 0; i < n_queries; i++) {
    h_query_labels[2*i] = query_label_vecs[i][0];
    if (query_label_vecs[i].size() == 1) {
      h_query_labels[2*i+1] = -1;
    } else {
      h_query_labels[2*i+1] = query_label_vecs[i][1];
    }
  }

  auto query_labels = raft::make_device_matrix<int64_t, int64_t>(res, n_queries, 2);
  raft::resource::sync_stream(res);
...
void compute_recall(const raft::host_matrix_view<uint32_t, int64_t>& neighbors,
  ...
  std::ofstream outfile("/u/cmo1/Filtered/cuvs/examples/cpp/src/result/yfcc_if_recall_results.csv");
  if (!outfile.is_open()) {
  ...
  for (int i = 0; i < neighbors.extent(0); i++) {
    ...
    for (int j = 0; j < topk; j++) {
      ...
      if (std::find(gt_indices[i].begin(), gt_indices[i].end(), neighbor_idx) != gt_indices[i].end()) {
        matches++;
      }
    }
  ...
  std::cout << "Results have been written to yfcc_if_recall_results.csv" << std::endl;
}
...
void cagra_build_search_variants(shared_resources::configured_raft_resources& dev_resources,
  ...
  std::string index_file = "/scratch/bdes/cmo1/CAGRA/indices_yfcc/single_indices/raft_index_"+ 
  ...
  if (test_file.good()) {
    index = cagra::deserialize<uint8_t, uint32_t>(dev_resources, index_file);
  ...
  auto batch_queries = raft::make_device_matrix<uint8_t, int64_t>(dev_resources, batch_size, dim);
